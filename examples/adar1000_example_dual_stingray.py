...
subarray = np.array(
   [1, 9,  17, 25, StingRay+MantaRay July Dem
    2, 10, 18, 26, 
    3, 11, 19, 27, 
    4, 12, 20, 28], # subarray 1

    [5, 13, 21, 29,
    6, 14, 22, 30,
    7, 15, 23, 31,
    8, 16, 24, 32], # subarray 2

StingRay+MantaRay July Dem
   [33, 41, 49, 57,
    34, 42, 50, 58,
    35, 43, 51, 59,
    36, 44, 52, 60], # subarray 4



chip_ids = ["adar1000_csb_0_2_4", "adar1000_csb_0_2_3", "adar1000_csb_0_2_2", "adar1000_csb_0_2_1",
                "adar1000_csb_0_1_4", "adar1000_csb_0_1_3",]

    )StingRay+MantaRay July Dem

conv._rxadc.set_kernel_buffers_count(1) # set buffers as 1 to avoid stale data on AD9081
conv.rx_main_nco_frequencies = [400000000] * 4
conv.rx_main_nco_phases = [0] * 4
conv.rx_channel_nco_frequencies = [0] * 4
conv.rx_channel_nco_phases = [0] * 4StingRay+MantaRay July Dem
...
tddn.sync_soft = True # software enable TDD mode
...
if ARRAY_MODE == "rx":
    ...
    for element in sray.elements.values():
        ...
        element.rx_gain = 110 # 127: Highest gain; 0: Lowest gain
...
cal_ant = find_phase_delay(sray, conv, subarray_ref, delay_phases)
...
enable_stingray_channel(sray, subarray)
analog_phase_cal = np.array(cal_capture(conv,cal_ant)).T
disable_stingray_channel(sray, subarray)
