...
N_RX = len(dev.rx_channel_nco_frequencies["axi-ad9084-rx-hpc"]) * D
N_TX = len(dev.tx_channel_nco_frequencies["axi-ad9084-rx-hpc"]) * D

# Total number of CDDCs/CDUCs
NM_RX = len(dev.rx_main_nco_frequencies["axi-ad9084-rx-hpc"]) * D
NM_TX = len(dev.tx_main_nco_frequencies["axi-ad9084-rx-hpc"]) * D
...
for i in range(10):
    ...
    for r in range(2):
        # Collect data
        x = dev.rx()
        rx_nco_phases["axi-ad9084-rx1"], s_b = measure_and_adjust_phase_offset(
            x[0], x[1], rx_nco_phases["axi-ad9084-rx1"]
        )
        rx_nco_phases["axi-ad9084-rx2"], s_c = measure_and_adjust_phase_offset(
            x[0], x[2], rx_nco_phases["axi-ad9084-rx2"]
        )
        rx_nco_phases["axi-ad9084-rx3"], s_d = measure_and_adjust_phase_offset(
            x[0], x[3], rx_nco_phases["axi-ad9084-rx3"]
        )
        phase_b = str(rx_nco_phases["axi-ad9084-rx1"][0] / 1000) + "\t" + str(int(s_b))
        phase_c = str(rx_nco_phases["axi-ad9084-rx2"][0] / 1000) + "\t" + str(int(s_c))
        phase_d = str(rx_nco_phases["axi-ad9084-rx3"][0] / 1000) + "\t" + str(int(s_d))
        phases_a.insert(i, rx_nco_phases["axi-ad9084-rx-hpc"][0] / 1000)
        phases_b.insert(i, rx_nco_phases["axi-ad9084-rx1"][0] / 1000)
        phases_c.insert(i, rx_nco_phases["axi-ad9084-rx2"][0] / 1000)
        phases_d.insert(i, rx_nco_phases["axi-ad9084-rx3"][0] / 1000)
        so_a.insert(i, 0)
        so_b.insert(i, s_b)
        so_c.insert(i, s_c)
        so_d.insert(i, s_d)
