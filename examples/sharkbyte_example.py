...
# Plot 1: Median overlay - show signal consistency
ax1 = plt.subplot(3, 3, 1)
# Calculate median signal for both devices
median_dev1 = np.median(np.array(analysis_dev1), axis=0)
median_dev2 = np.median(np.array(analysis_dev2), axis=0)
# Show first 2000 samples for clarity
zoom_len = min(2000, len(median_dev1))
ax1.plot(median_dev1[:zoom_len], label='Device 1 (Median)', linewidth=2, alpha=0.8)
ax1.plot(median_dev2[:zoom_len], label='Device 2 (Median)', linewidth=2, alpha=0.8)
ax1.set_xlabel('Sample Index')
ax1.set_ylabel('ADC Value')
ax1.set_title(f'Median Signal Overlay ({num_analysis_captures} captures)')
ax1.legend()
ax1.grid(True, alpha=0.3)

# Plot 2: All captures overlay (semi-transparent to see density)
ax2 = plt.subplot(3, 3, 2)
for i in range(min(20, num_analysis_captures)):  # Show max 20 for visibility
    ax2.plot(analysis_dev1[i][:zoom_len], color='blue', alpha=0.1, linewidth=0.5)
    ax2.plot(analysis_dev2[i][:zoom_len], color='orange', alpha=0.1, linewidth=0.5)
ax2.plot(median_dev1[:zoom_len], color='blue', label='Dev1 Median', linewidth=2)
ax2.plot(median_dev2[:zoom_len], color='orange', label='Dev2 Median', linewidth=2)
ax2.set_xlabel('Sample Index')
ax2.set_ylabel('ADC Value')
ax2.set_title(f'Signal Overlay (first 20 of {num_analysis_captures})')
ax2.legend()
...
plt.tight_layout()
plt.savefig("Phase_Shift_Analysis_Multiple_Captures.png", dpi=150)
print(f"\nPhase shift analysis plot saved as: Phase_Shift_Analysis_Multiple_Captures.png")
plt.show()
plt.close()
