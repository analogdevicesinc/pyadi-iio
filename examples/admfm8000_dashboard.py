...
class DigitalRampTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.freq_lbl.setProperty("class", "caption")
        ctrl_layout.addWidget(self.freq_lbl)
        self.freq_spin = QDoubleSpinBox()
        self.freq_spin.setRange(0.0, 10000.0)
        self.freq_spin.setDecimals(3)
        self.freq_spin.setSingleStep(1.0)
        self.freq_spin.setValue(10.0)
        self.freq_spin.setSuffix(" kHz")
        ctrl_layout.addWidget(self.freq_spin)
        self._update_freq_label(self.freq_spin.value())
    ...
    def _connect_signals(self):
        ...
        self.freq_spin.valueChanged.connect(self._update_freq_label)
        self.freq_spin.valueChanged.connect(lambda: self._update_preview())
    ...
    def _update_freq_label(self, freq_khz):
        if freq_khz > 0:
            period_us = 1e3 / freq_khz
            self.freq_lbl.setText(f"Ramp Frequency (kHz) — period {period_us:.2f} us")
        else:
            self.freq_lbl.setText("Ramp Frequency (kHz)")
    ...
    def _build_drctl_pulses(self, T, burst_count, burst_delay):
        """Build DRCTL pulse train (rising_t, falling_t list) and total duration.

        burst_delay is appended to the last period's low phase of each burst,
        so DRCTL remains low for (T - duty*T) + burst_delay between bursts.
        """
        half = self._drctl_duty * T
        pulses = []
        t = 0.0
        if burst_count > 0:
            n_bursts = 2
    ...
    def _drive_toward(self, t_segs, f_segs, t_start, duration, f_start, target, rate):
        """Drive freq from f_start toward target at given slew rate over duration."""
        if duration <= 0:
            return t_start, f_start
        t_end = t_start + duration
        if f_start == target or rate <= 0:
            t_segs.append(np.linspace(t_start, t_end, 50))
            f_segs.append(np.full(50, f_start))
            return t_end, f_start
        direction = 1.0 if target > f_start else -1.0
        time_to_reach = abs(target - f_start) / rate
        if duration <= time_to_reach:
            f_end = f_start + direction * rate * duration
            t_segs.append(np.linspace(t_start, t_end, 100))
            f_segs.append(np.linspace(f_start, f_end, 100))
            return t_end, f_end
        t_reach = t_start + time_to_reach
        t_segs.append(np.linspace(t_start, t_reach, 100))
        f_segs.append(np.linspace(f_start, target, 100))
        t_segs.append(np.linspace(t_reach, t_end, 50))
        f_segs.append(np.full(50, target))
        return t_end, target

    def _render_edge_triggered(self, t_segs, f_segs, edges, total, edge_kind,
                                  ramp_time, f_min, f_max):
        """Render RAMP_UP / RAMP_DOWN by triggering ramps on DRCTL edges.

        Missed triggers (device still ramping) are dropped.
        """
        if edge_kind == "up":
            idle_level = f_min
            ramp_dir = "up"
        ...
        for edge_t in edges:
            ...
            if edge_t > t_off:
                t_off = self._add_dwell(
                    t_segs, f_segs, edge_t - t_off, idle_level, t_off,
                )
            t_off = self._add_ramp(
                t_segs, f_segs, ramp_dir, f_min, f_max,
                ramp_time, ramp_time, t_off,
            )
            next_idle = t_off
        if t_off < total:
            self._add_dwell(t_segs, f_segs, total - t_off, idle_level, t_off)

    def _render_bidirectional(self, t_segs, f_segs, pulses, total,
                                inc, dec, f_min, f_max):
        """BIDIRECTIONAL: freq tracks DRCTL — drives toward f_max when high, f_min when low."""
        inc_rate = (f_max - f_min) / inc if inc > 0 else float("inf")
        dec_rate = (f_max - f_min) / dec if dec > 0 else float("inf")
        # Build DRCTL level segments
        segments = []
        last_t = 0.0
        for r, f_edge in pulses:
            if last_t < r:
                segments.append((last_t, r, 0))
            segments.append((r, f_edge, 1))
            last_t = f_edge
        if last_t < total:
            segments.append((last_t, total, 0))
        t_off = 0.0
        f_current = f_min
        for ts, te, level in segments:
            target = f_max if level == 1 else f_min
            rate = inc_rate if level == 1 else dec_rate
            t_off, f_current = self._drive_toward(
                t_segs, f_segs, t_off, te - ts, f_current, target, rate,
            )
    ...
    def _update_preview(self):
        ...
        freq_khz = self.freq_spin.value()
        if freq_khz <= 0:
            return
        ramp_period = 1.0 / (freq_khz * 1e3)
        ...
        if mode_key == "BIDIRECTIONAL_CONTINUOUS":
            ...
            pulses, total = self._build_drctl_pulses(
                ramp_period, burst_count, burst_delay,
            )
            if mode_key == "RAMP_UP":
                edges = [r for r, _ in pulses]
                self._render_edge_triggered(
                    t_segs, f_segs, edges, total, "up",
                    inc_time, f_min, f_max,
                )
            elif mode_key == "RAMP_DOWN":
                edges = [f for _, f in pulses]
                self._render_edge_triggered(
                    t_segs, f_segs, edges, total, "down",
                    dec_time, f_min, f_max,
                )
            else:  # BIDIRECTIONAL
                self._render_bidirectional(
                    t_segs, f_segs, pulses, total,
                    inc_time, dec_time, f_min, f_max,
                )
        ...
        self.plot.plot(t_all, f_all, pen=pg.mkPen(self._theme.categorical[0], width=2))
    ...
    def _on_apply(self):
        ...
        inc_time = self.inc_time_spin.value() * 1e-6
        dec_time = self.dec_time_spin.value() * 1e-6
        frequency = 0.0 if mode_key == "BIDIRECTIONAL_CONTINUOUS" else self.freq_spin.value() * 1e3
        burst_count = self.burst_count_spin.value()
