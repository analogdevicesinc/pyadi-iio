...
class Ltc2992:
  ...
  def power_sequencer_enable(self):
    ...
    device = 'stingray0_control'
    ch = 5
    val = self._iio.write_dev_ch_attr(dev_name = device, channel = 'voltage' + str(ch), attr = 'raw', val=1)
    print(val)
  ...
  def power_sequencer_power_good(self):
    device = 'stingray0_control'
    ch = 5
    val = self._iio.read_dev_ch_attr(dev_name = device, channel = 'voltage' + str(ch), attr = 'raw')
    print(val)
...
class GPIOS:
  ...
  def gpio_pulse(self, iio, ch, powerup=True):
...
class Stingray:
  ...
  def pulse_power_pin(self, which, powerup=True):
    if which.lower() == 'pwr_up_down':
      self._gpio.gpio_pulse(self._iio, self._gpio.PWR_UP_DOWN_PIN, powerup)
    elif which.lower() == '5v_ctrl':
      self._gpio.gpio_pulse(self._iio, self._gpio.P5V_CTRL_PIN, powerup)
    else:
  ...
  def powerdown(self):
    ...
    if self.fully_powered():
      ...
      self.pulse_power_pin('5v_ctrl', False)
    ...
    if self.partially_powered():
      ...
      self.pulse_power_pin('pwr_up_down', False)
  ...
  def powerup(self, enable_5v=True, **kwargs):
    ...
    if not self.partially_powered():
      self.pulse_power_pin('pwr_up_down', True)
    ...
    if enable_5v and not self.fully_powered():
      self.pulse_power_pin('5v_ctrl', True)
