...
def talise_init(my_talise):
    ...
    my_talise.load_phase_cal_fmcomms8()
    my_talise.load_gain_cal_fmcomms8()
    if((config.used_rx_channels > 0) and (config.used_rx_channels <= 8)):
        my_talise.num_rx_elements_fmcomms8 = config.used_rx_channels
    else:
        print("WARNING: Wrong number of used_rx_channels! Modify config file!")
        my_talise.num_rx_elements_fmcomms8 = 8
    ...
    my_talise.trx_lo_chip_c = config.lo_freq
    my_talise.trx_lo_chip_d = config.lo_freq
    ...
    my_talise.tx_hardwaregain_chan0_chip_c = config.tx_unused_channel_gain
    my_talise.tx_hardwaregain_chan1_chip_c = config.tx_unused_channel_gain
    my_talise.tx_hardwaregain_chan0_chip_d = config.tx_unused_channel_gain
    my_talise.tx_hardwaregain_chan1_chip_d = config.tx_unused_channel_gain
    ...
    my_talise.gain_control_mode_chan0_chip_c = config.rx_gain_control_mode
    my_talise.gain_control_mode_chan1_chip_c = config.rx_gain_control_mode
    my_talise.gain_control_mode_chan0_chip_d = config.rx_gain_control_mode
    my_talise.gain_control_mode_chan1_chip_d = config.rx_gain_control_mode
    ...
    my_talise.rx_hardwaregain_chan0_chip_c = config.rx_gain
    my_talise.rx_hardwaregain_chan1_chip_c = config.rx_gain
    my_talise.rx_hardwaregain_chan0_chip_d = config.rx_gain
    my_talise.rx_hardwaregain_chan1_chip_d = config.rx_gain
    ...
    my_talise.calibrate_rx_qec_en_chip_c = 1
    my_talise.calibrate_rx_qec_en_chip_d = 1
    my_talise.calibrate_tx_qec_en = 1
    my_talise.calibrate_tx_qec_en_chip_b = 1
    my_talise.calibrate_tx_qec_en_chip_c = 1
    my_talise.calibrate_tx_qec_en_chip_d = 1
    my_talise.calibrate_rx_phase_correction_en_chip_b = 1
    my_talise.calibrate_rx_phase_correction_en_chip_c = 1
    my_talise.calibrate_rx_phase_correction_en_chip_d = 1
    my_talise.calibrate_rx_phase_correction_en = 1
    my_talise.calibrate = 1
    my_talise.calibrate_chip_b = 1
    my_talise.calibrate_chip_c = 1
    my_talise.calibrate_chip_d = 1
...
def adjust_gain(talise_obj, *args):
    ...
    if (len(args) == config.used_rx_channels):
        for i, samples in enumerate(args):
            adjusted_samples.append(samples * talise_obj.gcal_fmcomms8[i])
...
def adjust_phase(talise_obj, *args):
    ...
    if (len(args) == (config.used_rx_channels - 1)):
        for i, samples in enumerate(args):
            phase_rad = np.deg2rad(talise_obj.pcal_fmcomms8[i])
            print("Phase of " + str(talise_obj.pcal_fmcomms8[i]) + " to radians = " + str(phase_rad))
...
def do_cal_gain(my_talise):
    ...
    time.sleep(1) # wait for internal calibrations
    # Adjust phase
    arrays_adjusted = adjust_phase(my_talise, rx_samples[1], rx_samples[2], rx_samples[3], rx_samples[4], rx_samples[5], rx_samples[6], rx_samples[7])
    for i in range(my_talise.num_rx_elements_fmcomms8 - 1):
    ...
    plt.plot(np.imag(rx_samples[0]), label = "Ch0 I (Imag)")

    plt.plot(np.real(rx_samples[1]), label = "Ch1 I (Real)")
    plt.plot(np.imag(rx_samples[1]), label = "Ch1 I (Imag)")

    plt.plot(np.real(rx_samples[2]), label = "Ch2 I (Real)")
    plt.plot(np.imag(rx_samples[2]), label = "Ch2 I (Imag)")

    plt.plot(np.real(rx_samples[3]), label = "Ch3 I (Real)")
    plt.plot(np.imag(rx_samples[3]), label = "Ch3 I (Imag)")
    
    plt.plot(np.real(rx_samples[4]), label = "Ch4 I (Real)")
    plt.plot(np.imag(rx_samples[4]), label = "Ch4 I (Imag)")

    plt.plot(np.real(rx_samples[5]), label = "Ch5 I (Real)")
    plt.plot(np.imag(rx_samples[5]), label = "Ch5 I (Imag)")

    plt.plot(np.real(rx_samples[6]), label = "Ch6 I (Real)")
    plt.plot(np.imag(rx_samples[6]), label = "Ch6 I (Imag)")

    plt.plot(np.real(rx_samples[7]), label = "Ch7 I (Real)")
    plt.plot(np.imag(rx_samples[7]), label = "Ch7 I (Imag)")
    ...
    amplitudes = [1.0] * (my_talise.num_rx_elements_fmcomms8)
    max_amplitude = 0
    elem_with_max_amplitude = 0
    # Save received amplitudes from each channel
    for i in range(my_talise.num_rx_elements_fmcomms8):
    ...
    amplitude_cal_coeff = [1.0] * (my_talise.num_rx_elements_fmcomms8)
    ...
    my_talise.gcal_fmcomms8 = amplitude_cal_coeff
    print("Gain calibration coefficents: " + str(amplitude_cal_coeff))
    my_talise.save_gain_cal_fmcomms8()
    my_talise.load_gain_cal_fmcomms8()

    # Adjust gain
    arrays_adjusted = adjust_gain(my_talise, rx_samples[0], rx_samples[1], rx_samples[2], rx_samples[3], rx_samples[4], rx_samples[5], rx_samples[6], rx_samples[7])
    for i in range(my_talise.num_rx_elements_fmcomms8):
...
def do_cal_phase(my_talise):
    ...
    ph_diff_ch0_minus_ch4 = []
    ph_diff_ch0_minus_ch5 = []
    ph_diff_ch0_minus_ch6 = []
    ph_diff_ch0_minus_ch7 = []

    avg_ph_diff_ch0_minus_ch1 = 0
    avg_ph_diff_ch0_minus_ch2 = 0
    avg_ph_diff_ch0_minus_ch3 = 0
    avg_ph_diff_ch0_minus_ch4 = 0
    avg_ph_diff_ch0_minus_ch5 = 0
    avg_ph_diff_ch0_minus_ch6 = 0
    avg_ph_diff_ch0_minus_ch7 = 0

    sum_ph_diff_ch0_minus_ch1 = 0
    sum_ph_diff_ch0_minus_ch2 = 0
    sum_ph_diff_ch0_minus_ch3 = 0
    sum_ph_diff_ch0_minus_ch4 = 0
    sum_ph_diff_ch0_minus_ch5 = 0
    sum_ph_diff_ch0_minus_ch6 = 0
    sum_ph_diff_ch0_minus_ch7 = 0
    ...
    max_ph_diff_ch0_minus_ch4 = 0
    min_ph_diff_ch0_minus_ch4 = 0
    max_ph_diff_ch0_minus_ch5 = 0
    min_ph_diff_ch0_minus_ch5 = 0
    max_ph_diff_ch0_minus_ch6 = 0
    min_ph_diff_ch0_minus_ch6 = 0
    max_ph_diff_ch0_minus_ch7 = 0
    min_ph_diff_ch0_minus_ch7 = 0
    ...
    for i in range(repeat_ph_calculations):
        ...
        print("Ph Diff Between ch0 and ch4: " + str(measure_phase_degrees(rx_samples[0], rx_samples[4])))
        ph_diff_ch0_minus_ch4.append(measure_phase_degrees(rx_samples[0], rx_samples[4]))
        print("Ph Diff Between ch0 and ch5: " + str(measure_phase_degrees(rx_samples[0], rx_samples[5])))
        ph_diff_ch0_minus_ch5.append(measure_phase_degrees(rx_samples[0], rx_samples[5]))
        print("Ph Diff Between ch0 and ch6: " + str(measure_phase_degrees(rx_samples[0], rx_samples[6])))
        ph_diff_ch0_minus_ch6.append(measure_phase_degrees(rx_samples[0], rx_samples[6]))
        print("Ph Diff Between ch0 and ch7: " + str(measure_phase_degrees(rx_samples[0], rx_samples[7])))
        ph_diff_ch0_minus_ch7.append(measure_phase_degrees(rx_samples[0], rx_samples[7]))
        ...
        sum_ph_diff_ch0_minus_ch4 += ph_diff_ch0_minus_ch4[i]
        sum_ph_diff_ch0_minus_ch5 += ph_diff_ch0_minus_ch5[i]
        sum_ph_diff_ch0_minus_ch6 += ph_diff_ch0_minus_ch6[i]
        sum_ph_diff_ch0_minus_ch7 += ph_diff_ch0_minus_ch7[i]
    ...
    avg_ph_diff_ch0_minus_ch4 = sum_ph_diff_ch0_minus_ch4/repeat_ph_calculations
    avg_ph_diff_ch0_minus_ch5 = sum_ph_diff_ch0_minus_ch5/repeat_ph_calculations
    avg_ph_diff_ch0_minus_ch6 = sum_ph_diff_ch0_minus_ch6/repeat_ph_calculations
    avg_ph_diff_ch0_minus_ch7 = sum_ph_diff_ch0_minus_ch7/repeat_ph_calculations
    
    my_talise.pcal_fmcomms8 = [avg_ph_diff_ch0_minus_ch1, avg_ph_diff_ch0_minus_ch2, avg_ph_diff_ch0_minus_ch3, avg_ph_diff_ch0_minus_ch4, avg_ph_diff_ch0_minus_ch5, avg_ph_diff_ch0_minus_ch6, avg_ph_diff_ch0_minus_ch7]
    print("pcal values: " + str(my_talise.pcal_fmcomms8))
    my_talise.save_phase_cal_fmcomms8()
    my_talise.load_phase_cal_fmcomms8()
    print("pcal values after save: " + str(my_talise.pcal_fmcomms8))
    ...
    print("Avg ph diff for ch0 - ch4: " + str(avg_ph_diff_ch0_minus_ch4))
    print("Avg ph diff for ch0 - ch5: " + str(avg_ph_diff_ch0_minus_ch5))
    print("Avg ph diff for ch0 - ch6: " + str(avg_ph_diff_ch0_minus_ch6))
    print("Avg ph diff for ch0 - ch7: " + str(avg_ph_diff_ch0_minus_ch7))
    ...
    max_ph_diff_ch0_minus_ch4 = max(ph_diff_ch0_minus_ch4)
    min_ph_diff_ch0_minus_ch4 = min(ph_diff_ch0_minus_ch4)
    max_ph_diff_ch0_minus_ch5 = max(ph_diff_ch0_minus_ch5)
    min_ph_diff_ch0_minus_ch5 = min(ph_diff_ch0_minus_ch5)
    max_ph_diff_ch0_minus_ch6 = max(ph_diff_ch0_minus_ch6)
    min_ph_diff_ch0_minus_ch6 = min(ph_diff_ch0_minus_ch6)
    max_ph_diff_ch0_minus_ch7 = max(ph_diff_ch0_minus_ch7)
    min_ph_diff_ch0_minus_ch7 = min(ph_diff_ch0_minus_ch7)
    ...
    print("Max diff in phase ch0-ch4: " + str(max_ph_diff_ch0_minus_ch4))
    print("Max diff in phase ch0-ch5: " + str(max_ph_diff_ch0_minus_ch5))
    print("Max diff in phase ch0-ch6: " + str(max_ph_diff_ch0_minus_ch6))
    print("Max diff in phase ch0-ch7: " + str(max_ph_diff_ch0_minus_ch7))
    ...
    print("Min diff in phase ch0-ch4: " + str(min_ph_diff_ch0_minus_ch4))
    print("Min diff in phase ch0-ch5: " + str(min_ph_diff_ch0_minus_ch5))
    print("Min diff in phase ch0-ch6: " + str(min_ph_diff_ch0_minus_ch6))
    print("Min diff in phase ch0-ch7: " + str(min_ph_diff_ch0_minus_ch7))

    print("Max variance: " + str(max((max_ph_diff_ch0_minus_ch1 - min_ph_diff_ch0_minus_ch1), (max_ph_diff_ch0_minus_ch2 - min_ph_diff_ch0_minus_ch2), \
                                     (max_ph_diff_ch0_minus_ch3 - max_ph_diff_ch0_minus_ch3), (max_ph_diff_ch0_minus_ch4 - min_ph_diff_ch0_minus_ch4), \
                                     (max_ph_diff_ch0_minus_ch5 - min_ph_diff_ch0_minus_ch5), (max_ph_diff_ch0_minus_ch6 - min_ph_diff_ch0_minus_ch6), \
                                     (max_ph_diff_ch0_minus_ch7 - min_ph_diff_ch0_minus_ch7))))
...
try:
    ...
    my_talise = adi.adrv9009_zu11eg_fmcomms8(uri=config.talise_address)
    print("Found Talise.")
...
if func == "cal_phase":
    ...
    for i in range(len(my_talise.pcal_fmcomms8)):
        print("PhCh0 - " + "PhCh" + str(i+1) + ": " + str(my_talise.pcal_fmcomms8[i]))

    # Print Gain calibraiton coefficients
    for i in range(len(my_talise.gcal_fmcomms8)):
        print("Gain coeff ch: " + str(i) + ": " + str(my_talise.gcal_fmcomms8[i]))   
    # Performing Beamforming
    plt.ion()
    print("Starting, use control-c to stop")
    
    phase_cal = [0, my_talise.pcal_fmcomms8[0], my_talise.pcal_fmcomms8[1], my_talise.pcal_fmcomms8[2], \
                 my_talise.pcal_fmcomms8[3], my_talise.pcal_fmcomms8[4], my_talise.pcal_fmcomms8[5], \
                 my_talise.pcal_fmcomms8[6]]
    ...
    print("Number of elements: " + str(my_talise.num_rx_elements_fmcomms8))
    ...
    try:
        while True:
            ...
            for phase in np.arange(-360/config.lambda_over_d_spacing, 360/config.lambda_over_d_spacing, 5): # sweep over angle
                ...
                data_sum = rx_samples[0] + rx_samples[1] + rx_samples[2] + rx_samples[3] + rx_samples[4] + rx_samples[5] + rx_samples[6] + rx_samples[7]# sum the two subarrays (within each subarray the 4 channels have already been summed)
