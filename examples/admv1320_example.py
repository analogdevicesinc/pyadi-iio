...
"""ADMV1320 Microwave Upconverter Example

Demonstrates configuration of the ADMV1320 transmit upconverter using pyadi-iio.

The ADMV1320 converts baseband/IF signals to RF signals across four selectable
RF bands (0-2 GHz, 1-5 GHz, 3-10 GHz, 6-20 GHz). It includes digitally
selectable attenuators (DSAs), IF common mode voltage control, configurable
LO sideband/filter, and filter/gain LUT support.

For standalone use, set the URI to the target device.
For Neponset board use, specify the device instance name (e.g., "admv1320_tx_0").
"""
...
# Connect to ADMV1320
# Standalone:
# tx = adi.admv1320(uri="ip:192.168.1.100")
# Neponset board (specify device instance):
tx = adi.admv1320(uri="ip:10.48.65.253", device_name="admv1320_tx_0")
