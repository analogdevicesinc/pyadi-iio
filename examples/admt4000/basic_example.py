...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ADMT4000 basic example")
    parser.add_argument(
        "--uri",
        type=str,
    ...
    if enable_plot:
        ...
        def animate(i, turns, angle):
            ...
            ax1.text(
                0.02, 0.95, f"Turns: {turns[-1]}", transform=ax1.transAxes,
            )
            ax1.set_title("Turns")
            ...
            ax2.text(
                0.02, 0.95, f"Angle: {angle[-1]:.4f} °", transform=ax2.transAxes,
            )
            ax2.set_title("Angle")
