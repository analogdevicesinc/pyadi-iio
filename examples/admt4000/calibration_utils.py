...
def print_cal_plots(
    ...
    angle_error,
    error_mag,
    corrected_angle_error=None,
    corrected_error_mag=None,
    plot_name="Angle Errors",
    blocking=True,
    show_computed=True,
):
    ...
    # plt.ylim(np.min(angle_error) * 1.5,
    #          np.max(angle_error) * 1.5)
    # plt.legend()
    ...
    plt.annotate(
        "H1 (11, " + str(round(error_mag[11], 4)) + ")",
        xy=(11, error_mag[11]),
        xytext=(0, 1.1),
    ...
    plt.annotate(
        "H2 (22, " + str(round(error_mag[22], 4)) + ")",
        xy=(22, error_mag[22]),
        xytext=(0.2, 1.1),
        textcoords="axes fraction",
        arrowprops=dict(facecolor="black", width=1, headwidth=3),
    ...
    # plt.ylim(np.min(error_mag) * 1.5,
    #          np.max(error_mag) * 1.5)
    # plt.ylim(10e-4, 10e0)  # Adjust the limits as necessary
    # plt.legend()
    if (
        show_computed
        and corrected_angle_error is not None
        and corrected_error_mag is not None
    ):
        plt.subplot(5, 1, 4)
        ...
        # plt.ylim(
        #     np.min(corrected_angle_error) * 1.5,
        #     np.max(corrected_angle_error) * 1.5,
        # )
        # plt.legend()
...
def calculate_corrections(angle_error, harmonic_error, ref_angles):
    ...
    for index in harmonic_error.keys():
        hx_correction = hx_correction + harmonic_error[index]["magnitude"] * np.sin(
            index * np.radians(ref_angles) + np.radians(harmonic_error[index]["phase"])
        )

    corrected_angle_error = angle_error - hx_correction
    angle_error_len = len(angle_error)
    corrected_error_fft = np.fft.fft(angle_error - hx_correction) / (
        angle_error_len / 2
    )
...
def harmonic_calibration(
    primary_angles, ref_angles, cycles=11, circulat_shift=0, CCW=0
):
    if CCW == 1:
    ...
    for index in harmonic_errors.keys():
        ...
        init_error = init_error + harmonic_errors[index]["magnitude"] * np.cos(
            np.radians(hx_phase)
        )
    ...
    for index in harmonic_errors.keys():
        harmonic_errors[index]["phase"] = (
            rot_dir * harmonic_errors[index]["phase"] - (index * init_angle - 90)
        ) % 360

    # scaling for ADMT4000
    mag_scale = 0.6072
    # mag_resolution = 360 / (2**16)  # ADMT4000 resolution in degrees
    mag_resolution = 0.005493
    phase_resolution = 0.087891

    for index in harmonic_errors.keys():
        scaled_harmonic_errors[index]["magnitude"] = int(
            harmonic_errors[index]["magnitude"] * mag_scale / mag_resolution
        )
        scaled_harmonic_errors[index]["phase"] = int(
            harmonic_errors[index]["phase"] / phase_resolution
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Harmonic calibration calculation for ADMT4000"
    )
    parser.add_argument(
        "--file",
        "-f",
        type=str,
        help="CSV file containing (columns: primary_angles, ref_angles (optional))",
    )
    parser.add_argument(
        "--calc_error",
        "-c",
        action="store_true",
        help="Calculate angle errors (requires 2 columns in CSV)",
    )
    ...
    if args.file:
        with open(args.file, "r") as csvfile:
            ...
            if args.calc_error:
                if len(data) < 2:
                    raise ValueError(
                        "CSV file must contain 2 columns when --calc_error is used"
                    )
                primary_angles = data[0]
                ref_angles = data[1]
            else:
                if len(data) < 1:
                    raise ValueError("CSV file must contain at least 1 column")
                primary_angles = data[0]
                # Generate ref_angles based on number of samples
                num_samples = len(primary_angles)
                ref_angles = []
    else:
        # Use hardcoded data
        print("No CSV file provided. Using hardcoded data.")
        primary_angles = np.array(
            [
                359.63745,
                15.07874,
                30.58594,
    ...
    (
        ...
        angle_error,
        error_mag,
    ) = harmonic_calibration(primary_angles, ref_angles)
    print("Harmonic Errors (raw):")
    pprint.pprint(harmonic_errors)
    print("Scaled Harmonic Errors:")
    pprint.pprint(scaled_harmonic_errors)

    corrected_angle_error = None
    corrected_error_mag = None

    if args.calc_error:
        corrected_angle_error, corrected_error_mag = calculate_corrections(
            angle_error, harmonic_errors, ref_angles
        )
    ...
    print_cal_plots(
        ...
        corrected_error_mag=corrected_error_mag,
        show_computed=args.calc_error,
    )
