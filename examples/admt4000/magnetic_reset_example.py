...
from adi.admt4000ard1z import admt4000ard1z
...
if __name__ == "__main__":
    ...
    dev.motor_sensor_alignment = True
    print_current_position(motor=dev.motor, sensor=dev.sensor)

    print(
        "\nRotating to turn = {} and angle = {}°...".format(
            starting_turn, starting_angle
        )
    )
    dev.rotate_absolute_angle = starting_angle + starting_turn * FULL_TURN
    print_current_position(motor=dev.motor, sensor=dev.sensor)
    ...
    if run_sweep:
        ...
        # Ensure motor usteps and sensor angles are aligned before starting
        dev.motor_sensor_alignment = True
        if log_to_file:
        ...
        for step_direction, sweep in [(-1, (45, 0)), (1, (0, 45))]:
            ...
            print_current_position(motor=dev.motor, sensor=dev.sensor)
            dev.rotate_absolute_angle = start_angle
            print_current_position(motor=dev.motor, sensor=dev.sensor)
            ...
            for i in range(num_steps + 1):
                ...
                dev.rotate_relative_angle = quarter_turn
