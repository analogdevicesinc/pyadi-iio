...
# Set to tr_pulse_time for a radar echo search. Keep at zero for direct TX/RX
# loopback, where the received signal can arrive during TX.
...
# Quantize the PRI to a period represented by TX, RX, and TDD clocks.
...
# Quantize the TX/TR interval to whole TX samples.
...
# The carrier is chosen so the requested number of cycles fits in the pulse.
...
# Build the exact complex Barker waveform that will be transmitted. Mapping
# sample indices this way fills the complete pulse, including non-integer chip
# widths after sample quantization.
...
# Place the Barker waveform in a zero-padded frame.
...
# Plot the expected TDD timing signals.
...
for frame in range(frame_pulses_to_plot):
    ...
    sync_stop = frame_start + tdd_tx_offload_pulse_start + tdd_sync_pulse_raw
    tdd_tx_offload_pulse_train[
        frame_start + tdd_tx_offload_pulse_start:sync_stop
    ] = 1

# Transmit and capture multiple synchronized RX buffers.
sdr.tx_destroy_buffer()
sdr.tx([iq, iq])
tddn.sync_soft = 1

rx_ch0 = np.zeros((capture_range, rx_buffer_samples), dtype=np.complex64)
rx_ch1 = np.zeros((capture_range, rx_buffer_samples), dtype=np.complex64)
...
# Matched-filter correlation against only the nonzero Barker burst. Correlating
# the zero-padded frame allows noise to win at different offsets per capture.
...
for capture in rx_ch0:
    # Try both complex rotation directions. This handles the positive- and
    # negative-frequency conventions that can occur across the RX chain.
    direct_correlation = np.correlate(
        capture * demodulation_carrier,
        barker_template_rx,
        mode="valid",
    )
    image_correlation = np.correlate(
        capture * np.conj(demodulation_carrier),
        barker_template_rx,
        mode="valid",
    )
    window_energy = np.convolve(
        np.abs(capture) ** 2,
        np.ones(barker_rx_samples),
        mode="valid",
    )
    normalized_direct = np.abs(direct_correlation) / np.sqrt(
        np.maximum(window_energy * template_energy, np.finfo(float).eps)
    )
    normalized_image = np.abs(image_correlation) / np.sqrt(
        np.maximum(window_energy * template_energy, np.finfo(float).eps)
    )
    normalized_correlation = np.maximum(normalized_direct, normalized_image)
    # Fold scores from all frames onto one PRI. Random noise peaks move from
    # frame to frame, while a real Barker return repeats at one offset.
    ...
    frame_scores = normalized_correlation[frame_indices]
    folded_scores = np.mean(frame_scores, axis=0)
    folded_scores[:rx_search_start_samples] = 0
    peak_offset = int(np.argmax(folded_scores))
    peak_score = folded_scores[peak_offset]
    background_scores = folded_scores[rx_search_start_samples:]
    background_median = np.median(background_scores)
    peak_ratio = peak_score / max(background_median, np.finfo(float).eps)
    if (
        peak_score < minimum_correlation_score
        or peak_ratio < minimum_correlation_peak_ratio
    ):
        raise RuntimeError(
...
axes[3].set_title("RX Data with Barker Correlation")
