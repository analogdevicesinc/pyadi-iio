...
PA_Supplies.set_voltage(1,16)  ## 16V Vdd
PA_Supplies.set_current(1,0.5)  ## 40A current limit

#1.8 Volt TR Toggle **Note, moved to different power supply during testing so E36233A can operate at higher current**
...
## Set PA_ON to 0 to disable Tx on both SPI busses ##
print("Setting PA_ON to 0 to disable Tx on both SPI busses")
...
print("PA Bias ON and PA Bias OFF set to -4.8V for all channels")
dev.latch_tx_settings()
...
for element in dev.elements.values():
    ...
    #Set PA Bias ON for channel being calibrated to -4.8V
    mr.disable_pa_bias_channel(dev, value)
    element.tx_enable = True
    element.pa_bias_on = starting_value
    while PA_Supplies.measure_current(1) <= (150e-3):
        time.sleep(0.05)
        prev_val = element.pa_bias_on
        if prev_val >= -2.0:
            time.sleep(1)
            temp_val = element.pa_bias_on
            if temp_val >= -2.0:
                print(f"PA Bias ON voltage too high for element {element}, moving to next element")
        ...
        #ramp step
        target_val = prev_val + 10e-3
        
        element.pa_bias_on = target_val 
        # print(element.pa_bias_on)
        dev.latch_tx_settings()

    print(f"PA Bias ON voltage for element {element} is {element.pa_bias_on}V, current is {PA_Supplies.measure_current(1)}A")
    PA_Bias_Values.append(element.pa_bias_on)
    PA_Bias_Elements.append(value)
    tone_1 = SpecAn.get_marker_power(marker=1)
    detect_bias.append(tone_1)
    print(f"Output Power:{tone_1}dBm")
    mr.disable_pa_bias_channel(dev, value)
            
PA_Bias_dict_ref = np.array(PA_Bias_Elements).flatten()
PA_Bias_dict = mr.create_dict(PA_Bias_dict_ref, np.array(PA_Bias_Values))
print("PA_Bias_dict", PA_Bias_dict)
PA_Bias_0_toggle.attrs["raw"].value = "0"
PA_Bias_1_toggle.attrs["raw"].value = "0"
mr.disable_pa_bias_channel(dev)
print(PA_Bias_Values)
...
for i in range(2):
    ...
    sdr.tx_hardwaregain_chan0 = -7
    sdr.tx_hardwaregain_chan1 = -7
    sdr.tx_hardwaregain_chan0_chip_b= 0
    sdr.tx_hardwaregain_chan1_chip_b = -7
...
for i in range(16):
    ...
    mr.enable_pa_bias_channel(dev,subarray[0,i],PA_Bias_dict)
    time.sleep(wait)
    time.sleep(wait)
    ...
    mr.enable_pa_bias_channel(dev,subarray[1,i],PA_Bias_dict)
    time.sleep(wait)
    time.sleep(wait)
    tone_1 = SpecAn.get_marker_power(marker=1)
    ...
    mr.enable_pa_bias_channel(dev,subarray[2,i],PA_Bias_dict)
    time.sleep(wait)
    time.sleep(wait)
    ...
    mr.enable_pa_bias_channel(dev,subarray[3,i],PA_Bias_dict)
    time.sleep(wait)
    time.sleep(wait)
...
mr.enable_pa_bias_channel(dev, subarray[:, 1],PA_Bias_dict)
...
mr.enable_pa_bias_channel(dev, ref_channels[0],PA_Bias_dict)
...
for element in dev.elements.values():
    ...
    if value not in ref_channels:
        detect = []
        print(f"Element: {value}")
        mr.enable_pa_bias_channel(dev, value,PA_Bias_dict)
        mr.enable_pa_bias_channel(dev, ref_channels[0],PA_Bias_dict)
...
try:
    script_path = os.path.join(os.path.dirname(__file__), "Manta_Ray_Tx_BeamsteeringPlots.py")
