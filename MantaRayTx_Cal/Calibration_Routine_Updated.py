...
PA_Supplies.set_current(1,30)  ## 30A current limit
...
tries = 10  ## Add try loop to ensure PA Biases are set properly.  Sometimes they do not set correctly on first try due to SPI issues. 
for device in dev.devices.values():
    # device.mode = "rx"
    device.bias_dac_mode = "on"
...
## Work in progress for manual gain cal routine, ignore for now ##
...
#     print(element)
#     value = int(mr.strip_to_last_two_digits(str_channel))
#     element.tx_attenuator = 1 ## Attenuator bypass active high
#     element.tx_gain = 127
#     dev.latch_tx_settings() 

#     element_peak_power = [] 

#     ## Capture power for each element ##
#     mr.enable_pa_bias_channel(dev,[value])
#     time.sleep(0.5)
#     tone_0 = SpecAn.get_marker_power(marker=1)
#     element_peak_power = tone_0

#     if element_peak_power > -20:
#         element.tx_attenuator = 0 ## Attenuator active
#         tone_0 = SpecAn.get_marker_power(marker=1)
#         element_peak_power = tone_0
#     if element_peak_power < -35:
#         element.tx_gain = 127
#     if element_peak_power > -20:
#         element.tx_gain = 0
#     else:
#         while element_peak_power > -35:
#             element.tx_gain -= 1
#             dev.latch_tx_settings() 
#             # time.sleep(0.1)
#             tone_0 = SpecAn.get_marker_power(marker=1)
#             element_peak_power = tone_0
#             print("tx_gain: ", element.tx_gain)
