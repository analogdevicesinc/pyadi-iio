...
# REFACTORED VERSION: All plotting localized to update_plot() function
# FIXED: Removed Qt blocking - uses matplotlib ion() with plt.pause() for responsive window
import time
import importlib
...
def update_plot():
    """
    Unified plotting function for real-time visualization.
    Uses matplotlib's ion() mode with plt.pause() for responsive window.
    """
    global fig, ax
    
    if plot_state['sweep_phase'] == 'azimuth':
        # Real-time azimuth plotting
        ax.clear()
        x = angles[: plot_state['current_index'] + 1]
        for sa in steering_angles:
            y = peak_mags_az[sa][: plot_state['current_index'] + 1]
        ...
        if plot_state['current_index'] >= 0:
            all_data = np.concatenate([peak_mags_az[sa][: plot_state['current_index'] + 1] for sa in steering_angles])
            if len(all_data) > 0:
        ...
        plt.pause(0.01)  # Allow OS to process window events

    elif plot_state['sweep_phase'] == 'post_processing' and not plot_state['post_sweep_completed']:
        # Post-sweep: Save static plots
        timestamp = plot_state['timestamps']
        plt.ioff()
        ...
        for steering_angle in steering_angles:
            plt.figure(figsize=(10, 6))
            plt.plot(angles, peak_mags_el[steering_angle],
                     linestyle='dotted', label='Measured Data', 
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(f'/home/snuc/Desktop/Tx_elevation_pattern_{steering_angle}deg_{timestamp}.png')
            plt.close()

        print("Plots saved to /home/snuc/Desktop/")
        plot_state['post_sweep_completed'] = True
...
# Setup matplotlib for real-time plotting with responsive window
plt.ion()
fig, ax = plt.subplots(figsize=(10, 6))

# Data collection loop - plot updates after each gimbal position
plot_state['sweep_phase'] = 'azimuth'
for i in range(len(gimbal_positions)):
    ...
    # Update index and plot
    plot_state['current_index'] = i
    mbx.move(gimbal_motor, sweepstep)
    update_plot()  # Plot after each gimbal position

# Finish azimuth sweep
mbx.gotoZERO()
...
# === Post-processing and file saves ===
plot_state['sweep_phase'] = 'post_processing'
update_plot()  # Trigger post-processing plots

# Save MATLAB data
timestamp = plot_state['timestamps']
matlab_data = {}
matlab_data["angles"] = np.asarray(angles).reshape(-1, 1)

az_keys = sorted(peak_mags_az.keys(), key=float) if peak_mags_az else []
if az_keys:
    az_matrix = np.column_stack([np.asarray(peak_mags_az[k]).ravel() for k in az_keys])
    matlab_data["az_steering_angles"] = np.asarray(az_keys).astype(float).reshape(1, -1)
...
if el_keys:
    el_matrix = np.column_stack([np.asarray(peak_mags_el[k]).ravel() for k in el_keys])
    matlab_data["el_steering_angles"] = np.asarray(el_keys).astype(float).reshape(1, -1)
    matlab_data["el_power"] = el_matrix
else:
    matlab_data["el_steering_angles"] = np.zeros((1, 0))
    matlab_data["el_power"] = np.zeros((len(angles), 0))

out_path = f"/home/snuc/Desktop/beam_sweeps_{timestamp}.mat"
os.makedirs(os.path.dirname(out_path), exist_ok=True)
savemat(out_path, matlab_data, do_compression=True)
print(f"Saved MATLAB .mat data file to: {out_path}")

plt.show()  # Display final plot and keep window open
