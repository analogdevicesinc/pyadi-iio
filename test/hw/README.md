...
# The labgrid env is no longer committed in-repo: fetch it from the
# coordinator (the exact source CI uses), then point LG_ENV at it.
curl -fsSL "http://10.0.0.41:8000/api/places/mini2/env-yaml?tier=boot" -o /tmp/lg-mini2.yaml
...
1. Ensure the board tag is listed in `.github/supported-boards.yml`.
   ...
   place's `daughter-board` tag appears in that file. Nothing in this
   repo names a runner or a place — the coordinator owns that. Each
   place's `runner` tag selects its self-hosted runner, and its env
   yaml is generated by the coordinator on demand.
2. Register the place on the coordinator with the right tags
   (`daughter-board`, `carrier`, `runner`, `boot-strategy`) and the
   driver stack / boot Strategy for that carrier (BootFPGASoC for
   ZCU102/SD-mux, BootFPGASoCTFTP for ZC706/TFTP, BootFabric for
   VCU118/JTAG). The coordinator then serves the labgrid env at
   `/api/places/<place>/env-yaml`; CI and local runs both fetch it
   from there — no in-repo env yaml.
