...
class max14906(context_manager):
    ...
    def __init__(self, uri=""):
        ...
        for ch in self._ctrl.channels:
            ...
            setattr(self, ch.id, self._channel(self._ctrl, ch.id, ch.output))
    ...
    class _channel(attribute):
        ...
        def do_mode(self):
            if not self.output:
                return 0
            return self._get_iio_attr_str(self.name, "do_mode", self.output)
        ...
        def do_mode_available(self):
            if not self.output:
                return 0
            return self._get_iio_attr_str(self.name, "do_mode_available", self.output)

        @property
        def IEC_type(self):
            if self.output:
                return 0
            return self._get_iio_attr_str(self.name, "IEC_Type", self.output)
        ...
        def IEC_type_available(self):
            if self.output:
                return 0
            return self._get_iio_attr_str(self.name, "IEC_Type_available", self.output)
