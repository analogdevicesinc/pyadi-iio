...
ssh.connect(hostname="192.168.0.1", port=22, username="root", password="analog")
...
while(True):
    ...
    bias_level = int(input("Press Any key change PA Bias and continue..."))

    for i in range(64):
        i = i + 1
        # time.sleep(0.25)
        print("Setting element " + str(i) + " PA bias to -2")
        dev.elements[i].pa_bias_on = bias_level
        print("Load Tx Setting")
        dev.latch_tx_settings()
