...
class ad7124(rx_chan_comp):  # noqa: D203,D211,D212,D213
    ...
    ``channel[]`` or as ``my_ad7124.voltage0_voltage1`` (hyphens sanitized to
    underscores to form valid Python identifiers).

    The ``enable_single_cycle`` device attribute defaults to 'Y', and ensures that when
    multiple channels are enabled, output data is fully settled; intermediate, unsettled
    outputs from the digital filter are ignored.
    Setting to 'N' includes these unsettled samples, and is thus only valid when
    a single channel is valid. WARNING: Setting to 'N' when multiple channels are
    enabled will result in invalid data.
