...
class admfm8000:
    ...
    @property
    def ramp_range(self):
        """Get the current DRG ramp frequency range in Hz

        This value represents the system-level frequency range of the DRG ramp
        when the DRG is enabled. It considers the current DRG frequency range
        settings and the overall frequency scaling factor based on the PLL
        settings.
        """
    ...
    @ramp_range.setter
    def ramp_range(self, value):
        if isinstance(value, tuple) and len(value) == 2:
            self.dds.drg.frequency.min = value[0] / self.frequency_scale
            self.dds.drg.frequency.max = value[1] / self.frequency_scale
        else:
            raise ValueError("Ramp range must be a tuple of (min_freq, max_freq)")

    @property
    def ramp_up_time(self):
        """Get/Set the DRG ramp up time in seconds

        This value represents the time it takes for the DRG to ramp from the
        minimum frequency to the maximum frequency. It is calculated based on
        the current DRG frequency range and increment step/rate settings.
        """
        freq_range = self.dds.drg.frequency.max - self.dds.drg.frequency.min
        if self.dds.drg.frequency.increment > 0:
            return freq_range / (self.dds.drg.increment_rate * self.dds.drg.frequency.increment)
        else:
            return 0

    @ramp_up_time.setter
    def ramp_up_time(self, value):
        freq_range = self.dds.drg.frequency.max - self.dds.drg.frequency.min
        inc_rate = self.dds.sysclk_frequency / 4  # max DRG update rate
        if value > 0:
            self.dds.drg.frequency.increment = freq_range / (inc_rate * value)
        else:
            self.dds.drg.frequency.increment = freq_range
        self.dds.drg.increment_rate = inc_rate

    @property
    def ramp_down_time(self):
        """Get/Set the DRG ramp down time in seconds

        This value represents the time it takes for the DRG to ramp from the
        maximum frequency to the minimum frequency. It is calculated based on
        the current DRG frequency range and decrement step/rate settings.
        """
        freq_range = self.dds.drg.frequency.max - self.dds.drg.frequency.min
        if self.dds.drg.frequency.decrement > 0:
            return freq_range / (self.dds.drg.decrement_rate * self.dds.drg.frequency.decrement)
        else:
            return 0

    @ramp_down_time.setter
    def ramp_down_time(self, value):
        freq_range = self.dds.drg.frequency.max - self.dds.drg.frequency.min
        dec_rate = self.dds.sysclk_frequency / 4  # max DRG update rate
        if value > 0:
            self.dds.drg.frequency.decrement = freq_range / (dec_rate * value)
        else:
            self.dds.drg.frequency.decrement = freq_range
    ...
    def ram_control_config(self, enable=True, frequency_np=None, output_bin=None):
        ...
        if frequency_np is not None:
            ...
            self.dds.ram.frequency_load(frequency_np / self.frequency_scale, output_bin)
