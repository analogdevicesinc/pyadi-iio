...
class ad5933(rx_chan_comp):
    ...
    def clock_frequency(self):
        """AD5933 system (master) clock frequency in Hz. Set this to the
        actual MCLK so the driver can convert sweep frequencies to register
        codes; internal oscillator is nominally 16.776 MHz."""
        return self._get_iio_debug_attr("clock_frequency", self._ctrl)

    @clock_frequency.setter
    def clock_frequency(self, value):
        self._set_iio_debug_attr_str("clock_frequency", value, self._ctrl)

    @property
    def minimum_output_frequency(self):
        """AD5933 minimum achievable excitation output frequency in Hz
        (read-only)."""
        return self._get_iio_debug_attr("minimum_output_frequency", self._ctrl)

    @property
    def maximum_output_frequency(self):
        """AD5933 maximum achievable excitation output frequency in Hz
        (read-only)."""
        return self._get_iio_debug_attr("maximum_output_frequency", self._ctrl)
