...
for sample_num in range(1, num_samples + 1):
    ...
    ax_full_array.set_title(f"RX Azimuth Pattern (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz", fontsize=13, fontweight='bold')
    ...
    sample_png_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_azimuth_pattern_{sig_gen_freq_GHz}GHz_{timestamp}.png"
    )
    ...
    ax_save.set_title(f'RX Azimuth Pattern - Full 64 Elements (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz', fontsize=14, fontweight='bold')
    ...
    normalized_png_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_azimuth_pattern_NORMALIZED_{sig_gen_freq_GHz}GHz_{timestamp}.png"
    )
    ...
    for angle in steering_angles:
        ax_norm.plot(angles, power_normalized[angle], 
                     linestyle='dotted', marker='o', markersize=5, linewidth=2, label=f'Sample {sample_num}')
    ax_norm.set_title(f'RX Azimuth Pattern NORMALIZED - Full 64 Elements (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz', 
...
ax_overlay.set_title(f'RX Azimuth Pattern Overlay - Full 64 Elements (All {num_samples} Samples) @ {sig_gen_freq_GHz} GHz', 
                     fontsize=14, fontweight='bold')
ax_overlay.set_xlabel('Mechanical Azimuth Angle (degrees)', fontsize=12)
...
overlay_png_path = os.path.join(base_out_dir, f"all_samples_overlay_{sig_gen_freq_GHz}GHz_{timestamp}.png")
...
ax_overlay_norm.set_title(f'RX Azimuth Pattern Overlay NORMALIZED - Full 64 Elements (All {num_samples} Samples) @ {sig_gen_freq_GHz} GHz', 
                          fontsize=14, fontweight='bold')
ax_overlay_norm.set_xlabel('Mechanical Azimuth Angle (degrees)', fontsize=12)
...
overlay_norm_png_path = os.path.join(base_out_dir, f"all_samples_overlay_NORMALIZED_{sig_gen_freq_GHz}GHz_{timestamp}.png")
...
all_samples_mat_path = os.path.join(base_out_dir, f"all_samples_combined_full_64element_{sig_gen_freq_GHz}GHz_{timestamp}.mat")
all_samples_dict = {
    ...
    "num_elements": 64,
    "description": "Repeatability test - all 64 elements combined, multiple samples",
