...
mray.steer_rx(azimuth=steering_angle, elevation=0) 
...
# Output folder setup
base_out_dir = "/home/snuc/Desktop/repeatability_test_all64"
...
for sample_num in range(1, num_samples + 1):
    ...
    # Initialize arrays for this sample - one for each subarray
    peak_mags_az = {}
    for subarray_name in subarrays.keys():
        peak_mags_az[subarray_name] = {angle: np.zeros(len(gimbal_positions)) for angle in steering_angles}
    ...
    # Left plot: Gimbal sweep - with one line per subarray
    colors_subarrays = ['darkblue', 'darkgreen', 'darkred', 'purple']
    subarray_lines = {}
    for idx, subarray_name in enumerate(subarrays.keys()):
        line, = ax_full_array.plot([], [], linestyle='dotted', marker='o', markersize=6, linewidth=2, 
                                   color=colors_subarrays[idx], label=subarray_name.replace('_', ' ').title())
        subarray_lines[subarray_name] = line
    
    ax_full_array.set_xlabel("Mechanical Azimuth Angle (degrees)", fontsize=12)
    ax_full_array.set_ylabel("Combined RF Input Power (dBm)", fontsize=12)
    ax_full_array.set_title(f"RX Azimuth Pattern - All 4 Subarrays (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz", fontsize=13, fontweight='bold')
    ...
    # === Azimuth Sweep for this sample ===
    print(f"Starting Azimuth Sweep for Sample {sample_num}...")
    ...
    for i in range(len(gimbal_positions)):
        ...
        for angle in steering_angles:
            mray.steer_rx(azimuth=angle, elevation=0)
            time.sleep(0.1)
            
            # Measure each subarray separately
            for subarray_name, subarray_elements in subarrays.items():
                # Enable only this subarray
                mr.disable_stingray_channel(mray)
                mr.enable_stingray_channel(mray, subarray_elements.tolist())
                time.sleep(0.3)
                ...
                peak_mags_az[subarray_name][angle][i] = mr.get_analog_mag(combined_data)
                
                # Collect IQ data and metadata for combined save (only once per position)
                if subarray_name == list(subarrays.keys())[0]:  # Only on first subarray
                    ...
                    sample_powers_64.append(peak_mags_az[subarray_name][angle][i])
            
            # Read temperatures at this measurement point (after all subarrays)
            ...
            for subarray_name in subarrays.keys():
                plot_power = peak_mags_az[subarray_name][angle][:i+1]
                subarray_lines[subarray_name].set_data(plot_angles, plot_power)
            ...
            valid_powers = []
            for subarray_name in subarrays.keys():
                valid_powers.extend(peak_mags_az[subarray_name][angle][:i+1][~np.isnan(peak_mags_az[subarray_name][angle][:i+1])])
            if len(valid_powers) > 0:
                y_min = np.min(valid_powers) - 5
                y_max = np.max(valid_powers) + 5
    ...
    # Normalize power data for this sample - for each subarray
    power_normalized = {}
    for subarray_name in subarrays.keys():
        power_normalized[subarray_name] = {}
        for angle in steering_angles:
            power_data = peak_mags_az[subarray_name][angle]
            max_power = np.max(power_data)
            # Normalize to 0 dB at the peak
            power_normalized[subarray_name][angle] = power_data - max_power
    
    # Save this sample's IQ data and temperature data to .mat file
    sample_iq_mat_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_raw_iq_full_64element_{sig_gen_freq_GHz}GHz_{timestamp}.mat"
    )
    ...
    # Get normalized data for first subarray (or combine all subarrays)
    first_subarray_name = list(subarrays.keys())[0]
    ...
    sample_iq_dict = {
        ...
        "power_normalized_dB": power_normalized[first_subarray_name][steering_angles[0]],
        "mechanical_angles_deg": angles,
        "frequency_GHz": sig_gen_freq_GHz,
        "description": f"Full 64-element combined RX sweep from -60 to +60 degrees with temperature monitoring (Sample {sample_num})",
    ...
    # Save this sample's power plot - all subarrays on one graph
    sample_png_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_subarrays_comparison_{sig_gen_freq_GHz}GHz_{timestamp}.png"
    )
    
    # Create and save the figure with all subarrays
    fig_save, ax_save = plt.subplots(figsize=(12, 7))
    colors_subarrays = ['darkblue', 'darkgreen', 'darkred', 'purple']
    for idx, subarray_name in enumerate(subarrays.keys()):
        for angle in steering_angles:
            ax_save.plot(angles, peak_mags_az[subarray_name][angle], 
                         linestyle='dotted', marker='o', markersize=5, linewidth=2, 
                         label=subarray_name.replace('_', ' ').title(),
                         color=colors_subarrays[idx])
    
    ax_save.set_title(f'RX Azimuth Pattern - All 4 Subarrays (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz', 
                      fontsize=14, fontweight='bold')
    ax_save.set_xlabel('Mechanical Azimuth Angle (degrees)', fontsize=12)
    ...
    plt.close(fig_save)
    print(f"Saved Sample {sample_num} subarrays comparison plot to: {sample_png_path}")
    
    # Save the normalized plot for this sample - all subarrays normalized
    normalized_png_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_subarrays_comparison_NORMALIZED_{sig_gen_freq_GHz}GHz_{timestamp}.png"
    )
    
    fig_norm, ax_norm = plt.subplots(figsize=(12, 7))
    for idx, subarray_name in enumerate(subarrays.keys()):
        for angle in steering_angles:
            ax_norm.plot(angles, power_normalized[subarray_name][angle], 
                         linestyle='dotted', marker='o', markersize=5, linewidth=2, 
                         label=subarray_name.replace('_', ' ').title(),
                         color=colors_subarrays[idx])
    
    ax_norm.set_title(f'RX Azimuth Pattern NORMALIZED - All 4 Subarrays (Sample {sample_num}) @ {sig_gen_freq_GHz} GHz', 
                      fontsize=14, fontweight='bold')
    ...
    plt.close(fig_norm)
    print(f"Saved Sample {sample_num} NORMALIZED subarrays plot to: {normalized_png_path}")
    
    # Save the dual-plot (gimbal + temperature) as well
    dual_plot_path = os.path.join(
        base_out_dir,
        f"sample_{sample_num:02d}_gimbal_and_temperature_{sig_gen_freq_GHz}GHz_{timestamp}.png"
    )
...
# Create overlay plot with all samples - comparing subarrays across sweeps
...
for subarray_idx, (subarray_name, ax) in enumerate(zip(subarrays.keys(), axes_overlay)):
    for sample_idx, sample_powers in enumerate(all_samples_powers, 1):
        ...
        temp_label = f'Sweep {sample_idx}: {peak_avg_temp:.1f}°C'
        
        for angle in steering_angles:
            power_data = sample_powers[subarray_name][angle]
            ax.plot(angles, power_data, 
                   linestyle='dotted', marker='o', markersize=4, linewidth=1.5, 
                   label=temp_label, color=colors[sample_idx - 1], alpha=0.8)
    
    ax.set_title(f'{subarray_name.replace("_", " ").title()} - All Samples @ {sig_gen_freq_GHz} GHz', 
                fontsize=12, fontweight='bold')
    ax.set_xlabel('Mechanical Azimuth Angle (degrees)', fontsize=11)
    ax.set_ylabel('Combined RF Input Power (dBm)', fontsize=11)
    ax.set_xlim([-(maxsweepangle/2), (maxsweepangle/2)])
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=9, loc='best')

fig_overlay.suptitle(f'RX Azimuth Pattern Overlay - All Subarrays & Samples @ {sig_gen_freq_GHz} GHz', 
                     fontsize=14, fontweight='bold')
fig_overlay.tight_layout()

overlay_png_path = os.path.join(base_out_dir, f"all_subarrays_all_samples_overlay_{sig_gen_freq_GHz}GHz_{timestamp}.png")
plt.savefig(overlay_png_path, dpi=150)
plt.close(fig_overlay)
print(f"\nSaved subarrays overlay plot to: {overlay_png_path}")

# Create normalized overlay plot with all samples and subarrays
fig_overlay_norm, axes_overlay_norm = plt.subplots(2, 2, figsize=(16, 12))
axes_overlay_norm = axes_overlay_norm.flatten()

for subarray_idx, (subarray_name, ax) in enumerate(zip(subarrays.keys(), axes_overlay_norm)):
    for sample_idx, sample_powers in enumerate(all_samples_powers, 1):
        # Get temperature info for this sample
        peak_avg_temp = all_samples_temp_stats[sample_idx - 1]
        temp_label = f'Sweep {sample_idx}: {peak_avg_temp:.1f}°C'
        ...
        for angle in steering_angles:
            # Normalize each sweep for this subarray
            power_data = sample_powers[subarray_name][angle]
            max_power = np.max(power_data)
            power_norm = power_data - max_power
            
            ax.plot(angles, power_norm, 
                   linestyle='dotted', marker='o', markersize=4, linewidth=1.5, 
                   label=temp_label, color=colors[sample_idx - 1], alpha=0.8)
    
    ax.set_title(f'{subarray_name.replace("_", " ").title()} NORMALIZED - All Samples @ {sig_gen_freq_GHz} GHz', 
                fontsize=12, fontweight='bold')
    ax.set_xlabel('Mechanical Azimuth Angle (degrees)', fontsize=11)
    ax.set_ylabel('Normalized Power (dB, 0 dB at peak)', fontsize=11)
    ax.set_xlim([-(maxsweepangle/2), (maxsweepangle/2)])
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=9, loc='best')

fig_overlay_norm.suptitle(f'RX Azimuth Pattern Overlay NORMALIZED - All Subarrays & Samples @ {sig_gen_freq_GHz} GHz', 
                          fontsize=14, fontweight='bold')
fig_overlay_norm.tight_layout()

overlay_norm_png_path = os.path.join(base_out_dir, f"all_subarrays_all_samples_overlay_NORMALIZED_{sig_gen_freq_GHz}GHz_{timestamp}.png")
plt.savefig(overlay_norm_png_path, dpi=150)
plt.close(fig_overlay_norm)
print(f"Saved NORMALIZED subarrays overlay plot to: {overlay_norm_png_path}")

# Create combined .mat file with all samples
all_samples_mat_path = os.path.join(base_out_dir, f"all_samples_combined_subarrays_{sig_gen_freq_GHz}GHz_{timestamp}.mat")

# Organize subarray data across all samples
subarray_data_dict = {}
for subarray_name in subarrays.keys():
    subarray_powers = []
    for sample_powers in all_samples_powers:
        subarray_powers.append(sample_powers[subarray_name][steering_angles[0]])
    subarray_data_dict[f"{subarray_name}_power_dBm"] = np.array(subarray_powers)

all_samples_dict = {
    "mechanical_angles_deg": angles,
    "frequency_GHz": sig_gen_freq_GHz,
    "num_samples": num_samples,
    "num_elements_per_subarray": len(subarrays[list(subarrays.keys())[0]]),
    "description": "Repeatability test - all 4 subarrays individually measured, multiple samples",
    "steering_angle_deg": 0,
    **subarray_data_dict
}
savemat(all_samples_mat_path, all_samples_dict, do_compression=True)
print(f"Saved combined samples .mat file (subarray data) to: {all_samples_mat_path}")
