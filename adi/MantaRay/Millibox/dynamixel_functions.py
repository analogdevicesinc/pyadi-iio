...
# Try multiple locations for the dynamixel library
possible_paths = [
    "/home/snuc/Desktop/libdxl_x64_c.so",
    os.path.join(os.path.dirname(__file__), "libdxl_x64_c.so"),
    "/usr/lib/libdxl_x64_c.so",
    "/usr/local/lib/libdxl_x64_c.so",
]

dxl_lib = None
for lib_path in possible_paths:
    try:
        dxl_lib = cdll.LoadLibrary(lib_path)
        print(f"Loaded Dynamixel library from: {lib_path}")
        break
    except (OSError, FileNotFoundError):
        continue

if dxl_lib is None:
    raise OSError(f"Could not find libdxl_x64_c.so in any of: {possible_paths}")
