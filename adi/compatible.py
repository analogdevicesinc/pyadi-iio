...
class compatible(context_manager, metaclass=ABCMeta):
    ...
    when using compatible.with_class() to create derived classes with specific
    compatible matching.
    """

    @staticmethod
    def with_class(cls: Type[C], named: str = "", **chip_info) -> Type[C]:
        ...
        :param chip_info:
            Additional attributes to add to the new class. This can be used to set
            class-level attributes that are specific to the compatible device.
        """
        ...
        chip_info["_compatible"] = named
        chip_info["__module__"] = inspect.stack()[1].frame.f_globals["__name__"]
        return type(named, (cls,), chip_info)
