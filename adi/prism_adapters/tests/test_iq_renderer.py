...
from adi.prism_adapters.iq import (
    IQRenderer, PayloadError, analyze, render_spectrum,
)
...
def _one_tone_payload(fs=1e6, f=100e3, n=8192):
    t = np.arange(n) / fs
    iq = (np.cos(2 * np.pi * f * t) + 1j * np.sin(2 * np.pi * f * t)).astype(np.complex64)
    return {
        "iq": iq, "fs": fs, "domain": "complex",
        "expected_tones": [f], "metrics": {"peak_min": -10},
    }
...
def test_render_spectrum_returns_html():
    res = analyze(_one_tone_payload())
    html = render_spectrum(res, _one_tone_payload(),
